...
# It's designed to run before the chain starts to properly initialize the data directory in case `fud init` was not run.
# This behaviour should probably live in tendermint.

configDir=$HOME/.nemo/config
dataDir=$HOME/.nemo/data
